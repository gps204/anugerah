...
@section('title', __('cms.social_media_links.create'))

@section('content')
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('cms.social_media_links.create') }}</h1>
    </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.social-media-links.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="translations[{{ $defaultLocale }}][platform_name]" id="platform_name" class="form-control @error('translations.'.$defaultLocale.'.platform_name') is-invalid @enderror" value="{{ old('translations.'.$defaultLocale.'.platform_name') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="url" name="link" id="link" class="form-control @error('link') is-invalid @enderror" value="{{ old('link') }}" required placeholder="https://">
                    @error('link')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <button type="submit" class="btn btn-primary">{{ __('cms.social_media_links.save') }}</button>
                </div>
            </form>
        </div>
    </div>
